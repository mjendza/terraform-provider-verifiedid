---
layout: "msgraph"
page_title: "MSGraph Provider: Authenticating via Managed Identity"
description: |-
  This guide will cover how to use managed identity for MSGraph resources as authentication for the MSGraph Provider.
---
...
Using managed identity for Azure resources as authentication is enabled in `msgraph` provider by default.

By default, Terraform will use the system assigned identity for authentication. To use a user assigned identity instead, you will need to specify the `ARM_CLIENT_ID` environment variable (equivalent to provider block argument [`client_id`](https://registry.terraform.io/providers/microsoft/msgraph/latest/docs#client_id)) to the [client id](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/user_assigned_identity#client_id) of the identity.
